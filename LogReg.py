...
from ReportCard import ReportCard, PlotReport, PlotReportSave
...
def LogModVec(filename, outcome, textName, count=True):
    """Just Vectorizing, if not count, then TF-IDF
    """
    df = pd.read_csv(filename+'.csv', sep=",")
...
if __name__ == "__main__":
    ...
    iotbl = baseline(iot, 'ScoreLabel')
    aibl = baseline(ai, 'ScoreLabel')
    statsbl = baseline(stats, 'ScoreLabel')
